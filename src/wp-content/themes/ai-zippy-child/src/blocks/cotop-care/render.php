...
$wrapper = get_block_wrapper_attributes(['class' => 'ctcare ctprod-section', 'data-product-section' => 'care']);
?>
<section <?php echo $wrapper; ?>>
    <?php if ($label) : ?><div class="ctcare__label"><?php echo esc_html($label); ?></div><?php endif; ?>
    <h2 class="ctcare__title"><?php echo esc_html($title); ?></h2>
    <?php if ($body) : ?><p class="ctcare__intro"><?php echo esc_html($body); ?></p><?php endif; ?>
    <div class="ctcare__grid">
        <?php foreach ($items as $item) : ?>
            <div class="ctcare__card">
                <div class="ctcare__icon"><?php echo function_exists('ai_zippy_cotop_icon') ? ai_zippy_cotop_icon((string) ($item['icon'] ?? 'shield')) : ''; ?></div>
                <div><h4><?php echo esc_html($item['title'] ?? ''); ?></h4><p><?php echo esc_html($item['body'] ?? ''); ?></p><a class="ctcare__link" href="<?php echo esc_url($item['url'] ?? '#'); ?>"><?php echo esc_html($item['link'] ?? ''); ?></a></div>
            </div>
        <?php endforeach; ?>
    </div>
</section>
